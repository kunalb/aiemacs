...
;;; Load custom lisp (with autoloads)
...
(let ((autoloads-outdated
       (or (not (file-exists-p lisp-autoloads-file))
           (cl-some (lambda (el-file)
                      (file-newer-than-file-p el-file lisp-autoloads-file))
                    (directory-files lisp-dir t "\\.el$")))))
  (when autoloads-outdated
    (loaddefs-generate lisp-dir lisp-autoloads-file)))
(load lisp-autoloads-file 'noerror 'nomessage)

;;; Customization file
(setq custom-file (concat user-emacs-directory "custom.el"))
(load custom-file 'noerror)

;;; Path configuration (loaded immediately - needed early)
...
;;; Jupyter (IPython REPL with %autoreload support)
(use-package jupyter
  :commands (jupyter-run-repl jupyter-connect-repl)
  :bind (:map python-mode-map
         ("C-c j j" . jupyter-run-repl)
         ("C-c j c" . jupyter-eval-line-or-region)
         ("C-c j b" . jupyter-eval-buffer)
         ("C-c j r" . jupyter-eval-region)
         ("C-c j f" . jupyter-eval-defun)
         ("C-c j i" . jupyter-inspect-at-point)
         ("C-c j R" . jupyter-repl-restart-kernel))
  :config
  (defun jupyter-repl-setup-autoreload ()
    "Enable IPython autoreload in the current Jupyter REPL."
    (interactive)
    (jupyter-eval "%load_ext autoreload")
    (jupyter-eval "%autoreload 2")
    (message "autoreload enabled"))

  (defun jupyter-run-repl-with-autoreload ()
    "Start Jupyter REPL and enable autoreload."
    (interactive)
    (call-interactively #'jupyter-run-repl)
    ;; Give kernel time to start before enabling autoreload
    (run-at-time 2 nil #'jupyter-repl-setup-autoreload))

  (define-key python-mode-map (kbd "C-c j J") 'jupyter-run-repl-with-autoreload)
  (define-key python-mode-map (kbd "C-c j a") 'jupyter-repl-setup-autoreload))
...
(use-package markdown-mode
  ...
  (define-key markdown-mode-map (kbd "C-c d") 'md-insert-date-heading))
...
;;; Theme manipulation (desaturate-theme, invert-theme available via autoload)
